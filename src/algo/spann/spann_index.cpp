...
SpannCoreParams ParseCoreParams(const case_insensitive_map_t<Value> &options, idx_t dim) {
	SpannCoreParams p;
	p.dim = dim;
	...
	if (nlist_it != options.end()) {
		...
		if (v < 2) {
			throw BinderException("SPANN index 'nlist' must be >= 2 (got %lld)", (long long)v);
		}
		p.nlist = idx_t(v);
	}
	...
	if (nprobe_it != options.end()) {
		...
		if (v < 1) {
			throw BinderException("SPANN index 'nprobe' must be >= 1 (got %lld)", (long long)v);
		}
		p.nprobe = idx_t(v);
	}
	auto rc_it = options.find("replica_count");
	if (rc_it != options.end()) {
		const int64_t v = rc_it->second.GetValue<int64_t>();
		if (v < 1) {
			throw BinderException("SPANN index 'replica_count' must be >= 1 (got %lld)", (long long)v);
		}
		p.replica_count = idx_t(v);
	}
	auto cf_it = options.find("closure_factor");
	if (cf_it != options.end()) {
		Value cast_cf;
		string cast_err;
		if (!cf_it->second.DefaultTryCastAs(LogicalType::DOUBLE, cast_cf, &cast_err)) {
			throw BinderException("SPANN index 'closure_factor' must be a number (%s)", cast_err);
		}
		const double v = cast_cf.GetValue<double>();
		if (!(v >= 1.0)) {
			throw BinderException("SPANN index 'closure_factor' must be >= 1 (got %f)", v);
		}
		p.closure_factor = static_cast<float>(v);
	}
	if (p.nprobe > p.nlist) {
		p.nprobe = p.nlist;
	}
	if (p.replica_count > p.nlist) {
		p.replica_count = p.nlist;
	}
	return p;
}

// State stream layout:
//   u64 magic                     ("VNDXSPN2" — index-level)
//   u64 rerank_multiple
...
constexpr uint64_t kStateMagic = 0x324E50534E444E56ULL; // "VNDXSPN2"
...
void SpannIndex::InitFromOptions(const case_insensitive_map_t<Value> &options, idx_t vector_size, MetricKind metric) {
	metric_ = metric;
	dim_ = vector_size;
	...
	core_ = make_uniq<SpannCore>(params_, *quantizer_, *store_);
}

SpannIndex::SpannIndex(const string &name, IndexConstraintType index_constraint_type,
	...
	if (index_constraint_type != IndexConstraintType::NONE) {
		throw NotImplementedException("SPANN indexes do not support unique or primary key constraints");
	}
	...
	if (vector_child_type.id() != LogicalTypeId::FLOAT) {
		throw NotImplementedException("SPANN indexes currently support only FLOAT[] vectors");
	}
...
void SpannIndex::WriteStateStream() {
	if (state_root_.Get() == 0) {
		state_root_ = BlockId {};
	}
...
void SpannIndex::ReadStateStream(BlockId root) {
	auto reader = store_->OpenStream(root);

	uint64_t magic = 0;
	reader->Read(reinterpret_cast<data_ptr_t>(&magic), sizeof(magic));
	if (magic != kStateMagic) {
		throw InternalException("SpannIndex: unrecognized state stream (magic mismatch)");
	}
...
idx_t SpannIndex::GetVectorSize() const {
	return dim_;
}

MetricKind SpannIndex::GetMetricKind() const {
	return metric_;
}

idx_t SpannIndex::GetRerankMultiple(ClientContext &context) const {
	Value v;
...
struct SpannIndexScanState : public IndexScanState {
	idx_t current_row = 0;
	idx_t total_rows = 0;
...
idx_t SpannIndex::Scan(IndexScanState &state, Vector &result, idx_t result_offset) {
	auto &scan_state = state.Cast<SpannIndexScanState>();
	idx_t count = 0;
...
struct SpannMultiScanState final : IndexScanState {
	Vector vec;
	vector<row_t> row_ids;
	idx_t nprobe;
	explicit SpannMultiScanState(idx_t nprobe_p) : vec(LogicalType::ROW_TYPE, nullptr), nprobe(nprobe_p) {
	}
};

unique_ptr<IndexScanState> SpannIndex::InitializeMultiScan(ClientContext &context) {
	const idx_t nprobe = ResolveNprobe(context, params_.nprobe);
	return make_uniq<SpannMultiScanState>(nprobe);
}

idx_t SpannIndex::ExecuteMultiScan(IndexScanState &state_p, float *query_vector, idx_t limit) {
	auto &state = state_p.Cast<SpannMultiScanState>();

	vector<float> query_workspace(quantizer_->QueryWorkspaceSize());
	quantizer_->PreprocessQuery(query_vector, query_workspace.data());

	vector<SpannCore::Candidate> cands;
	{
		auto lock = rwlock.GetSharedLock();
		const idx_t oversample = limit + tombstones_.size();
...
const Vector &SpannIndex::GetMultiScanResult(IndexScanState &state) {
	auto &scan_state = state.Cast<SpannMultiScanState>();
	FlatVector::SetData(scan_state.vec, (data_ptr_t)scan_state.row_ids.data());
	return scan_state.vec;
}

void SpannIndex::ResetMultiScan(IndexScanState &state) {
	auto &scan_state = state.Cast<SpannMultiScanState>();
...
void SpannIndex::CommitDrop(IndexLock &index_lock) {
	auto lock = rwlock.GetExclusiveLock();
	core_.reset();
	row_to_centroid_.clear();
	tombstones_.clear();
...
void SpannIndex::TrainQuantizer(ColumnDataCollection &collection, idx_t sample_cap) {
	auto lock = rwlock.GetExclusiveLock();
	vector<float> buf;
...
void SpannIndex::TrainCentroids(ColumnDataCollection &collection, idx_t sample_cap) {
	auto lock = rwlock.GetExclusiveLock();
	vector<float> buf;
	const idx_t n = SampleFloats(collection, dim_, sample_cap, buf);
...
void SpannIndex::Construct(DataChunk &input, Vector &row_ids, idx_t /*thread_idx*/) {
...
void SpannIndex::Compact() {
	is_dirty = true;
	// Mirrors IvfIndex::Compact — rebuilding the replica postings from
	// tombstones would require decoding (and more fundamentally, the replica
	// set might shift under an unrelated centroid re-train). Scan already
	// filters tombstones; a real shrink wants REINDEX.
	auto lock = rwlock.GetExclusiveLock();
}

void SpannIndex::Delete(IndexLock &lock, DataChunk &input, Vector &rowid_vec) {
	is_dirty = true;
	const auto count = input.size();
...
ErrorData SpannIndex::Insert(IndexLock &lock, DataChunk &input, Vector &rowid_vec) {
	Construct(input, rowid_vec, 0);
	return ErrorData {};
}

ErrorData SpannIndex::Append(IndexLock &lock, DataChunk &appended_data, Vector &row_identifiers) {
...
IndexStorageInfo SpannIndex::SerializeToDisk(QueryContext context, const case_insensitive_map_t<Value> &options) {
	PersistToDisk();
	auto &block_manager = table_io_manager.GetIndexBlockManager();
...
IndexStorageInfo SpannIndex::SerializeToWAL(const case_insensitive_map_t<Value> &options) {
	PersistToDisk();
	IndexStorageInfo info = store_->GetInfo();
	info.name = name;
	info.root = state_root_.Get();
...
idx_t SpannIndex::GetInMemorySize(IndexLock &state) {
	return store_->GetInMemorySize();
}

bool SpannIndex::MergeIndexes(IndexLock &state, BoundIndex &other_index) {
	throw NotImplementedException("SpannIndex::MergeIndexes() not implemented");
}

void SpannIndex::Vacuum(IndexLock &state) {
}

void SpannIndex::Verify(IndexLock &state) {
}

string SpannIndex::ToString(IndexLock &state, bool display_ascii) {
	throw NotImplementedException("SpannIndex::ToString() not implemented");
}

void SpannIndex::VerifyAllocations(IndexLock &state) {
}

void SpannIndex::VerifyBuffers(IndexLock &lock) {
}
...
void RegisterIndex(DatabaseInstance &db) {
	...
	index_type.name = SpannIndex::TYPE_NAME;
	index_type.create_instance = [](CreateIndexInput &input) -> unique_ptr<BoundIndex> {
		auto res = make_uniq<SpannIndex>(input.name, input.constraint_type, input.column_ids, input.table_io_manager,
		                                 input.unbound_expressions, input.db, input.options, input.storage_info);
		return std::move(res);
	};
	index_type.create_plan = SpannIndex::CreatePlan;

	// The `vindex_*` extension options are shared across algorithms; whoever
	// registers first wins. `vindex_nprobe` is specifically claimed by IVF
	// but fits SPANN 1:1 (same semantics: how many posting lists to scan).
	...
	if (!db.config.GetOptionByName("vindex_nprobe")) {
		db.config.AddExtensionOption("vindex_nprobe",
		                             "override the nprobe parameter when scanning IVF/SPANN indexes",
		                             LogicalType::BIGINT);
	}

	db.config.GetIndexTypes().RegisterIndexType(index_type);

	VectorIndexRegistry::Instance().RegisterTypeName(SpannIndex::TYPE_NAME);
}
