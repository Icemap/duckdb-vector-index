...
PhysicalCreateDiskAnnIndex::PhysicalCreateDiskAnnIndex(PhysicalPlan &physical_plan,
...
// Global / Local State
//-------------------------------------------------------------
class CreateDiskAnnIndexGlobalState final : public GlobalSinkState {
public:
	explicit CreateDiskAnnIndexGlobalState(const PhysicalOperator &op_p) : op(op_p) {
	}

	const PhysicalOperator &op;
	unique_ptr<DiskAnnIndex> global_index;
...
unique_ptr<GlobalSinkState> PhysicalCreateDiskAnnIndex::GetGlobalSinkState(ClientContext &context) const {
	auto gstate = make_uniq<CreateDiskAnnIndexGlobalState>(*this);
	...
	auto &db = storage.db;
	gstate->global_index =
	    make_uniq<DiskAnnIndex>(info->index_name, constraint_type, storage_ids, table_manager, unbound_expressions, db,
	                            info->options, IndexStorageInfo(), estimated_cardinality);

	return std::move(gstate);
}

class CreateDiskAnnIndexLocalState final : public LocalSinkState {
public:
	unique_ptr<ColumnDataCollection> collection;
...
unique_ptr<LocalSinkState> PhysicalCreateDiskAnnIndex::GetLocalSinkState(ExecutionContext &context) const {
	auto state = make_uniq<CreateDiskAnnIndexLocalState>();
	vector<LogicalType> data_types = {unbound_expressions[0]->return_type, LogicalType::ROW_TYPE};
...
SinkResultType PhysicalCreateDiskAnnIndex::Sink(ExecutionContext &context, DataChunk &chunk,
	...
	auto &lstate = input.local_state.Cast<CreateDiskAnnIndexLocalState>();
	auto &gstate = input.global_state.Cast<CreateDiskAnnIndexGlobalState>();
...
class DiskAnnIndexConstructTask final : public ExecutorTask {
public:
	DiskAnnIndexConstructTask(shared_ptr<Event> event_p, ClientContext &context,
	                          CreateDiskAnnIndexGlobalState &gstate_p, size_t thread_id_p,
	                          const PhysicalCreateDiskAnnIndex &op_p)
...
private:
	CreateDiskAnnIndexGlobalState &gstate;
	size_t thread_id;
...
class DiskAnnIndexConstructionEvent final : public BasePipelineEvent {
public:
	DiskAnnIndexConstructionEvent(const PhysicalCreateDiskAnnIndex &op_p, CreateDiskAnnIndexGlobalState &gstate_p,
	                              Pipeline &pipeline_p, CreateIndexInfo &info_p,
	...
	const PhysicalCreateDiskAnnIndex &op;
	CreateDiskAnnIndexGlobalState &gstate;
	...
	void Schedule() override {
		...
		for (size_t tnum = 0; tnum < num_threads; tnum++) {
			construct_tasks.push_back(
			    make_uniq<DiskAnnIndexConstructTask>(shared_from_this(), context, gstate, tnum, op));
		}
...
SinkFinalizeType PhysicalCreateDiskAnnIndex::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	// Quantizer train must come before any Insert: DiskAnnCore::Insert encodes
	// through the quantizer, and PQ / RaBitQ need populated codebooks.
	...
	auto new_event =
	    make_shared_ptr<DiskAnnIndexConstructionEvent>(*this, gstate, pipeline, *info, storage_ids, table);
	event.InsertEvent(std::move(new_event));
	return SinkFinalizeType::READY;
}

ProgressData PhysicalCreateDiskAnnIndex::GetSinkProgress(ClientContext &context, GlobalSinkState &gstate,
                                                         ProgressData source_progress) const {
	ProgressData res;
	const auto &state = gstate.Cast<CreateDiskAnnIndexGlobalState>();
